    ...
    <div className="bg-gray-100 dark:bg-gray-900 min-h-screen flex flex-col">
        ...
        {messages.map((m, i) => (
          <div key={i} className={`flex ${m.me ? 'justify-end' : 'justify-start'}`}>
            <div className={`max-w-[70%] px-3 py-2 rounded-2xl text-sm ${m.me
              ...
              <p>{m.text}</p>
              <p className={`text-[9px] mt-1 ${m.me ? 'text-green-200' : 'text-gray-400'}`}>{m.time}</p>
            </div>
          </div>
        ))}
      </div>
      <div className="p-3 bg-white dark:bg-gray-800 border-t border-gray-200 dark:border-gray-700">
        <div className="flex gap-2">
          ...
          <button onClick={handleSend} className="w-9 h-9 rounded-full bg-green-500 flex items-center justify-center">
