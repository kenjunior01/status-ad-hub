...
export default function Login() {
  ...
  const onSubmit = async (values: LoginValues) => {
    ...
    if (error) {
      ...
      if (isPanic) {
        ...
        // v3.40.0 — na APK cai no Console Guardião (ecrã inicial app-first)
        navigate(isNativeApp() ? "/dashboard/inicio" : "/dashboard");
        return;
      }
      // Normal login error
  ...
  // v3.40.0 — inputs da variante nativa Tactical
  const tacInputCls = (hasError: boolean) =>
    `h-11 w-full rounded-xl border ${hasError ? 'border-red-500/50' : 'border-[rgba(52,211,153,0.15)] focus:border-[rgba(52,211,153,0.45)]'} bg-[rgba(52,211,153,0.03)] px-3.5 font-mono text-sm text-emerald-50 placeholder:text-emerald-100/20 outline-none transition-colors`

  // Modal de recuperação de senha — partilhado entre a variante WEB e a
  // variante NATIVA Tactical do login
  const resetModal = resetMode ? (
    <motion.div
      ...
      >
        <div className="flex items-center gap-3 mb-6">
          <div className="p-2.5 rounded-xl bg-brand/10 border border-brand/15">
            <MailWarning className="h-5 w-5 text-brand" />
          </div>
          <div>
        ...
        <input
          ...
          className="w-full h-11 rounded-xl border border-white/[0.08] bg-white/[0.03] pl-4 pr-4 text-white text-sm placeholder:text-white/20 outline-none focus-visible:ring-2 focus-visible:ring-brand/30 focus-visible:border-brand/30 transition-all duration-200 mb-4"
        />
        <div className="flex gap-3">
          ...
          <button
            ...
            className="flex-1 h-11 rounded-xl bg-brand hover:bg-brand-dark text-white text-sm font-semibold disabled:opacity-50 transition gap-2 flex items-center justify-center"
          >
  ...
  /* ── v3.40.0 — LOGIN NATIVO PRÓPRIO DA APK (identidade Tactical) ──
     Ecrã de app de verdade: compacto, sem painel de marketing, Google EM
     PRIMEIRO LUGAR (a MESMA conta da versão web). O duress tap (5 toques
     no escudo) mantém-se. Na web mantém-se o design dourado. */
  if (native) {
    return (
      <div className="dark tactical relative flex min-h-screen flex-col items-center justify-center px-5 py-10">
        <div className="tac-scanline pointer-events-none absolute inset-0" aria-hidden="true" />
        <div className="relative z-10 w-full max-w-sm">
          {/* Logo — mantém o duress tap (5 toques arma o modo disfarçado) */}
          <div className="mb-7 flex flex-col items-center text-center">
            <div
              className={`relative flex h-16 w-16 items-center justify-center rounded-2xl border cursor-pointer select-none transition-all ${tapFlash ? 'border-[rgba(52,211,153,0.55)] bg-[rgba(52,211,153,0.14)]' : 'border-[rgba(52,211,153,0.22)] bg-[rgba(52,211,153,0.06)]'}`}
              onClick={handleShieldTap} role="button" tabIndex={-1} aria-hidden="true"
            >
              <Shield className="h-8 w-8" style={{ color: 'var(--tac-green)' }} strokeWidth={1.6} />
              <span className="tac-dot absolute -top-0.5 -right-0.5" />
            </div>
            <h1 className="mt-4 font-mono text-xl font-bold tracking-[0.22em] text-emerald-50">GUARDIÃO</h1>
            <p className="mt-1.5 font-mono text-[9px] tracking-[0.24em] text-emerald-100/35">
              STATUSADS CONNECT · MESMA CONTA DA WEB
            </p>
          </div>

          <div className="tac-panel p-5">
            {/* GOOGLE em primeiro lugar — o login principal da APK */}
            <button
              type="button"
              disabled={oauthLoading === 'google'}
              onClick={() => handleOAuth('google')}
              className="tac-btn mb-4 w-full"
            >
            ...
            <div className="mb-4 flex items-center gap-2.5">
              <div className="h-px flex-1 bg-[rgba(52,211,153,0.12)]" />
              <span className="font-mono text-[8.5px] tracking-[0.22em] text-emerald-100/30">OU COM EMAIL</span>
              <div className="h-px flex-1 bg-[rgba(52,211,153,0.12)]" />
            </div>

            <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-3.5">
              <input id="email" type="email" placeholder="seu@email.com" className={tacInputCls(!!errors.email)} {...register("email")} />
              {errors.email && <p className="font-mono text-[10px] text-red-400/80">{errors.email.message}</p>}
              <input id="password" type={showPassword ? "text" : "password"} placeholder="••••••••" className={tacInputCls(!!errors.password)} {...register("password")} />
              {errors.password && <p className="font-mono text-[10px] text-red-400/80">{errors.password.message}</p>}
              <div className="flex justify-end">
                <button type="button" onClick={() => setShowPassword(!showPassword)} className="font-mono text-[10px] text-emerald-100/40 transition hover:text-emerald-100/70">
                  {showPassword ? 'ESCONDER SENHA' : 'MOSTRAR SENHA'}
                </button>
              </div>
              <button type="button" onClick={() => setResetMode(true)} className="text-left font-mono text-[10px] text-[var(--tac-green)]/70 transition hover:text-[var(--tac-green)]">
                ESQUECEU A SENHA?
              </button>
          ...
          <p className="mt-5 text-center font-mono text-[10px] text-emerald-100/30">
            Primeira vez?{' '}
            <Link to="/ativar" className="text-[var(--tac-green)]/80 transition hover:text-[var(--tac-green)]">ACTIVAR DISPOSITIVO</Link>
          </p>
        ...
        <motion.div initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 1, ease: [0.25, 0.4, 0.25, 1] }} className="relative z-10 flex flex-col items-center px-8 text-center">
          ...
          {isDuressArmed && (
            <motion.div
              ...
              aria-hidden="true"
            >
              <div className="w-2 h-2 rounded-full bg-brand shadow-[0_0_6px_rgba(212,175,55,0.6)] animate-pulse" />
            </motion.div>
          )}
          <motion.div
          ...
          >
            <div className={`flex h-28 w-28 items-center justify-center rounded-3xl border backdrop-blur-md shadow-[0_0_60px_-15px_rgba(212,175,55,0.15)] transition-all duration-200 ${tapFlash ? 'border-brand/50 bg-brand/[0.12]' : 'border-brand/20 bg-brand/[0.06]'}`}>
              <Shield className={`h-14 w-14 text-brand transition-transform duration-150 ${tapFlash ? 'scale-110' : ''}`} strokeWidth={1} />
            </div>
            <div className="absolute inset-0 rounded-3xl bg-brand/5 blur-2xl" />
          </motion.div>
          <h2 className="font-display text-3xl font-bold text-white">A Sua <span className="bg-gradient-to-r from-brand to-amber-300 bg-clip-text text-transparent">Seguranca</span> Comeca Aqui</h2>
        ...
        <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, ease: [0.25, 0.4, 0.25, 1] }} className="relative z-10 w-full max-w-md">
          ...
          <div className={`rounded-2xl border border-white/[0.08] bg-white/[0.02] backdrop-blur-2xl shadow-2xl shadow-black/20 ${native ? 'p-6' : 'p-8'}`}>
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="mt-8 flex flex-col gap-5">
              ...
              <div className="flex justify-end">
                <button type="button" onClick={() => setResetMode(true)} className="text-xs text-brand/70 transition hover:text-brand">Esqueceu a senha?</button>
              </div>
            ...
            <div className="flex gap-3">
              <button type="button" disabled={oauthLoading === 'google'} className="flex h-11 flex-1 items-center justify-center gap-2 rounded-xl border border-white/[0.08] bg-white/[0.03] text-white/60 text-sm transition-all hover:bg-white/[0.06] hover:text-white/80 hover:border-white/15 disabled:opacity-50" onClick={() => handleOAuth('google')}>
                {oauthLoading === 'google' ? <Loader2 className="h-4 w-4 animate-spin" /> : (<>
                <svg className="h-4 w-4" viewBox="0 0 24 24"><path d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92a5.06 5.06 0 01-2.2 3.32v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.1z" fill="#4285F4" /><path d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" fill="#34A853" /><path d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z" fill="#FBBC05" /><path d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" fill="#EA4335" /></svg>
                Google</>)}
              </button>
              <button type="button" disabled={oauthLoading === 'apple'} className="flex h-11 flex-1 items-center justify-center gap-2 rounded-xl border border-white/[0.08] bg-white/[0.03] text-white/60 text-sm transition-all hover:bg-white/[0.06] hover:text-white/80 hover:border-white/15 disabled:opacity-50" onClick={() => handleOAuth('apple')}>
          ...
          <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.5 }} className="mt-6 text-center text-sm text-white/30">
            Nao tem conta? <Link to="/ativar" className="font-medium text-brand/70 transition hover:text-brand">Activar Dispositivo</Link>
          </motion.p>

          {/* PASSWORD RESET MODAL — partilhado (também usado na variante nativa) */}
          {resetModal}
        </motion.div>
      </div>
    </div>
