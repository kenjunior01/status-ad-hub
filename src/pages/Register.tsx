...
import { Shield, Eye, EyeOff, Mail, Lock, User, Phone, ArrowRight, Users, Check, Loader2 } from "lucide-react";
...
export default function Register() {
  ...
  // v3.35.0 — Google adaptativo: a conta criada/ligada é a MESMA da web
  async function handleGoogle() {
    setGoogleLoading(true);
    const r = await signInWithOAuthAdaptive("google");
    setGoogleLoading(false);
    if (!r.ok) {
      toast.error("Login com Google indisponível", { description: r.message, duration: 7000 });
    }
  }
      ...
      <div className="flex flex-1 items-center justify-center px-4 py-12 relative">
        ...
        <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }} className="relative z-10 w-full max-w-md">
          ...
          <div className="rounded-2xl border border-white/[0.08] bg-white/[0.02] p-8 backdrop-blur-2xl shadow-2xl shadow-black/20">
            ...
            {/* v3.35.0 — continue com Google (paridade com o Login) */}
            ...
            <button
              ...
              className="flex h-11 w-full items-center justify-center gap-2 rounded-xl border border-white/[0.08] bg-white/[0.03] text-white/60 text-sm transition-all hover:bg-white/[0.06] hover:text-white/80 hover:border-white/15 disabled:opacity-50"
            >
