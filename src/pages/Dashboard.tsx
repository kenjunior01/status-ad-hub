...
export default function Dashboard() {
  ...
  // Mobile: menu lateral (a página Dashboard tapa o cabeçalho do layout, por isso
  // tem o seu próprio botão de menu) + bottom sheets com os painéis escondidos no telemóvel
  const [menuOpen, setMenuOpen] = useState(false)
  const [mobilePanel, setMobilePanel] = useState<null | 'devices' | 'security'>(null)
  ...
  // ── MOBILE: conteúdo das bottom sheets — no telemóvel os painéis laterais de
  // desktop ficam escondidos (hidden md:block), por isto dá-lhe acesso ao mesmo
  // informação através de duas sheets arrastáveis ("Dispositivos" e "Estado"). ──
  const mobileDevicesSheet = (
      ...
      <div>
        ...
        {displayDevices.length === 0 ? (
          <div className="text-center py-6">
            ...
            <button
              onClick={() => { setMobilePanel(null); navigate('/dashboard/devices') }}
              className="text-[11px] text-brand mt-2 font-semibold active:opacity-70"
            >Ir para Dispositivos</button>
          </div>
        ) : (
          <div className="space-y-2">
            {displayDevices.map(d => {
              ...
              return (
                <div key={d.id} className="flex items-center gap-3 p-2.5 rounded-xl bg-white/[0.02] border border-white/[0.04]">
      ...
      {(() => {
        ...
        return (
          <CoachCard
            ...
            onClick={() => { setMobilePanel(null); navigate('/dashboard/dicas') }}
          />
        )
      })()}
    </div>
  ...
  const mobileSecuritySheet = (
    <div className="space-y-4">
      {/* Cabeçalho: título + pill de prontidão */}
      ...
      {/* Barra de progresso de prontidão */}
      ...
      <div>
        ...
        <ResponsiveContainer width="100%" height={90}>
          <AreaChart data={hourlyData}>
            <defs><linearGradient id="greenGradM" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor="#D4AF37" stopOpacity={0.3} /><stop offset="100%" stopColor="#D4AF37" stopOpacity={0} /></linearGradient></defs>
            ...
            <Area type="monotone" dataKey="localizacoes" stroke="#D4AF37" strokeWidth={2} fill="url(#greenGradM)" />
          </AreaChart>
        </ResponsiveContainer>
      </div>
      {/* Monitoramento BLE */}
      ...
      <div className="pt-3 border-t border-white/[0.06]">
        ...
        {/* Distância ao centro da zona segura */}
    ...
    <div className="relative h-screen w-full overflow-hidden bg-background">
      ...
      >
        <div className="flex items-center gap-2.5">
          {/* Menu — a página Dashboard cobre o cabeçalho do layout, por isso precisa do próprio botão */}
          <button onClick={() => setMenuOpen(true)} className="p-2 -ml-2 rounded-xl hover:bg-white/[0.06] active:bg-white/10 transition" aria-label="Abrir menu">
      ...
      <AnimatePresence>
        {showNotifBanner && isReady && (
          <motion.div
            ...
            className="absolute top-[104px] md:top-16 inset-x-0 mx-auto w-[calc(100%-2rem)] max-w-md z-50"
          >
      ...
      {/* MOBILE: atalhos flutuantes para os painéis (Dispositivos / Estado) — só telemóvel */}
      {isReady && (
        <motion.div
          initial={{ y: 40, opacity: 0 }} animate={{ y: 0, opacity: 1 }}
          transition={{ delay: 0.35, type: 'spring', stiffness: 120 }}
          className="fixed bottom-20 left-3 right-3 z-30 flex gap-2 md:hidden"
        >
          <button
            onClick={() => setMobilePanel('devices')}
            className="flex-1 flex items-center justify-center gap-2 h-11 rounded-2xl bg-card/90 backdrop-blur-xl border border-white/[0.08] text-white/70 text-xs font-semibold active:scale-[0.98] transition-transform shadow-lg"
          >
            <BluetoothConnected className="h-4 w-4 text-brand" />
            Dispositivos
            <span className="px-1.5 py-0.5 rounded-md bg-brand/10 text-brand text-[10px] border border-brand/20">{activeDevices}/{totalDevices || 0}</span>
          </button>
          <button
          ...
          >
            ...
            <span className={cn(
              ...
              readinessScore >= 50 ? 'bg-brand/10 text-brand border-brand/20' : 'bg-red-500/10 text-red-400 border-red-500/20'
            )}>{readinessScore}%</span>
          </button>
        </motion.div>
      )}

      {/* MOBILE: bottom sheets com o conteúdo dos painéis (equivalente aos painéis de desktop) */}
      <Drawer open={mobilePanel === 'devices'} onOpenChange={(o) => setMobilePanel(o ? 'devices' : null)}>
        <DrawerContent className="bg-[#0D1321] border-white/10">
          <DrawerTitle className="sr-only">Dispositivos e Prontidao</DrawerTitle>
          <div className="overflow-y-auto px-4 pt-2 pb-10 max-h-[75vh]">
            {mobileDevicesSheet}
          </div>
        </DrawerContent>
      </Drawer>
      <Drawer open={mobilePanel === 'security'} onOpenChange={(o) => setMobilePanel(o ? 'security' : null)}>
        <DrawerContent className="bg-[#0D1321] border-white/10">
          <DrawerTitle className="sr-only">Estado de Seguranca</DrawerTitle>
          <div className="overflow-y-auto px-4 pt-2 pb-10 max-h-[75vh]">
            {mobileSecuritySheet}
          </div>
        </DrawerContent>
      ...
      {/* Menu lateral — a página Dashboard tem cabeçalho próprio que tapa o do layout */}
